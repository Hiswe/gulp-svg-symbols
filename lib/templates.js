...
function attributesToString(obj) {
  return Object.getOwnPropertyNames(obj).reduce((str, key) => {
    const attr = key.replace(/[^a-zA-Z-:]/g, ``)
    const raw = obj[key]
    const rawEscaped = String(raw).replace(/"/g, `&quot;`)
    const t = typeof raw
    if (t === `boolean` && raw === true) {
      return `${str} ${attr}`
    }
    if (t === `string` || (t === `number` && !isNaN(raw))) {
...
function render(template, datas) {
  ...
  return tmpl(template, datas).then(result => utils.createFile(name, result))
}
