...
function formatForTemplate(svgRawData, options) {
  ...
  var transformDataOptions  = _.omit(options, [
    'transformData',
    'templates'
  ]);
  ...
  if (options.title !== false) tmplDatas.title = utils.dynamicText(options.title, svgRawData.name);
  ...
  result            = options.transformData(svgRawData, tmplDatas, transformDataOptions);
